...
// QF-HRA-11 — Biên bản phỏng vấn nghỉ việc
export const useExitInterviewStore = defineStore('exitInterview', {
  state: () => ({
    list: [],
  ...
  actions: {
    async fetchList(params = {}) {
      ...
      try {
        ...
        qs.set('page_size', '30')
        const res = await useFetchAuth(`/v1/exit-interview?${qs}`)
        this.list = res?.data?.items ?? res?.data ?? []
        this.total = res?.data?.total ?? this.list.length
      } catch (e) {
        this.error = e?.data?.message ?? 'Không thể tải danh sách biên bản phỏng vấn'
      } finally {
        this.loading = false
      }
    },
    ...
    async fetchById(id) {
      ...
      this.error = null
      try {
        const res = await useFetchAuth(`/v1/exit-interview/${id}`)
    ...
    async create(payload) {
      ...
      this.error = null
      try {
        const res = await useFetchAuth('/v1/exit-interview', {
          method: 'POST',
          body: payload,
        })
    ...
    async takeActionWithContent(id, payload = {}) {
      ...
      this.error = null
      try {
        const res = await useFetchAuth(`/v1/exit-interview/${id}/actions`, {
          method: 'POST',
          body: payload,
        })
        ...
        try {
          ...
          if (processId) {
            ...
            triggerNotification({
              ...
              note: payload.note ?? '',
              extra: { document_type: 'EXIT_INTERVIEW', document_id: id },
            })
          }
        } catch (_) {}
