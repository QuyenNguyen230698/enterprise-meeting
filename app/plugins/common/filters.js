...
/**
 *  2024-12-17 AnhLD.
 * Format number with commas as thousands separators and optional decimal places.
 * @param {Number} number - Required
 * @param {Boolean} useDot - Optional
 * @returns {String}
 */
export const formatNumberWithCommas = (number, useDot = false) => {
  let formattedNumber = (number || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  
  if (useDot) {
    formattedNumber += ".00"; // Thêm .00 nếu useDot là true
  }
