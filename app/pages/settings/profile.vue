        ...
        <div class="flex-1 overflow-y-auto overscroll-contain bg-slate-50 pb-4">
          <div v-if="activeTab === 'profile'" class="px-4 sm:px-6 pt-4 pb-1 flex justify-end">
            <div class="flex items-center gap-2">
              <button
                @click="navigateTo('/sign-hub?tab=signature&action=open-signature')"
                class="px-3 py-2 text-xs font-semibold rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors flex items-center gap-1.5"
              >
                <i class="bi bi-pen"></i>
                {{ signatureStatus.has_signature ? 'Quản lý chữ ký' : 'Tạo chữ ký cá nhân' }}
              </button>
            </div>
          </div>
          <ProfileTabProfile
